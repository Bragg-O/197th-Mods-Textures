/*
	...
	Description:
	This code spawns an anonymous function that checks if a player has the "197th_BactaGrenade_Ammo" item in their backpack, vest, or uniform.
	If the player has the item, it starts a loop that checks every second if the player still has the item. If the player doesn't have the item after 120 seconds, the loop ends.
	if the player is a medic and still has one item, the loop waits 5 seconds and checks if the player still has the item. if the player still has the item, it displays an error message and plays an alarm sound.
	This function is meant to check if a player is using a bacta grenade, which is not allowed in-game.
	
	Parameter(s):
	0: Object - The unit object.
	1: Object - The container object.
	
	Examples:
	["_unit", "_container"] call CIM_fnc_checkBactaGrenade;
*/
_this spawn {
