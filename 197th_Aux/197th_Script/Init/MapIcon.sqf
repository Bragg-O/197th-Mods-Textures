...
_QS_ST_showOnlyVehicles = true;// BOOL. set true to show ONLY vehicles, no foot-soldier units will be shown. May override other config. default true.
...
QS_ST_STR_text1 = 'Cliquez pour afficher les détails du groupe';// STRING. text shown when a player passes Mouse over group leader (only if _QS_ST_groupInteractiveIcons = true;)
QS_ST_STR_text2 = 'Ce groupe ne fait pas partie de votre faction!';// STRING. text shown when a player clicks on a group Icon of other faction. (only if _QS_ST_groupInteractiveIcons = true;)
...
// =============================================================================================================================// 
	...
	_QS_fnc_isIncapacitated = {
		params ['_u', '_med'];
		if ((lifeState _u) isEqualTo 'INCAPACITATED') exitWith {
			true
		};
		private _r = false;
		if (_med isEqualTo 'BTC') then {
			if (!isNil {
				_u getVariable 'BTC_need_revive'
			}) then {
			...
			if (_med isEqualTo 'FAR') then {
				if (!isNil {
					_u getVariable 'FAR_isUnconscious'
				}) then {
				...
				if (_med isEqualTo 'AIS') then {
					if (!isNil {
						_u getVariable 'unit_is_unconscious'
					}) then {
					...
					if (_med isEqualTo 'AWS') then {
						...
						if (_med isEqualTo 'ACE') then {
							if (!isNil {
								_u getVariable 'ace_isUnconscious'
							}) then {
	...
	_QS_fnc_iconColor = {
		...
		if (_useTeamColor) then {
			if (isNull (objectParent _u)) then {
				...
				if (_s isEqualTo east) then {
					_c = _QS_ST_X select 9;
				};
				if (_s isEqualTo west) then {
					_c = _QS_ST_X select 10;
				};
				if (_s isEqualTo resistance) then {
					_c = _QS_ST_X select 11;
				};
				if (_s isEqualTo civilian) then {
		...
		if (_s isEqualTo east) exitWith {
			_c = _QS_ST_X select 9; _c set [3, _a];if (_ds isEqualTo 1) then {
				if (_ms > 0.80) then {
					_c set [3, 0];
				};
			};_c;
		};
		if (_s isEqualTo west) exitWith {
			_c = _QS_ST_X select 10;_c set [3, _a];if (_ds isEqualTo 1) then {
				if (_ms > 0.80) then {
					_c set [3, 0];
				};
			};_c;
		};
		if (_s isEqualTo resistance) exitWith {
			_c = _QS_ST_X select 11;_c set [3, _a];if (_ds isEqualTo 1) then {
				if (_ms > 0.80) then {
					_c set [3, 0];
				};
			};_c;
		...
		if (_s isEqualTo civilian) exitWith {
			_c = _QS_ST_X select 12;_c set [3, _a];if (_ds isEqualTo 1) then {
				if (_ms > 0.80) then {
					_c set [3, 0];
				};
			};_c;
		};
		_c = _QS_ST_X select 13;if (_ds isEqualTo 1) then {
			if (_ms > 0.80) then {
				_c set [3, 0];
			};
		};_c;
	...
	_QS_fnc_iconSize = {
		...
		if (_i isEqualTo 0) then {
			if (_v isKindOf 'Man') then {
				_i = _QS_ST_X select 22;_i;
			};
			if (_v isKindOf 'LandVehicle') then {
				_i = _QS_ST_X select 23;_i;
			};
			if (_v isKindOf 'Air') then {
				_i = _QS_ST_X select 25;_i;
			};
			if (_v isKindOf 'StaticWeapon') then {
				_i = _QS_ST_X select 26; _i;
			};
			if (_v isKindOf 'Ship') then {
				_i = _QS_ST_X select 24;_i;
			};
		...
		if (_ds isEqualTo 1) then {
			if (_dl > 0) then {
				if (diag_tickTime > (missionNamespace getVariable 'QS_ST_iconUpdatePulseTimer')) then {
					...
					if (!isNil {
						_v getVariable 'QS_ST_lastPulsePos'
					}) then {
	...
	_QS_fnc_iconText = {
		...
		if (((_v distance2D player) < (_QS_ST_X select 68)) || {
			(_isAdmin)
		}) then {
			if ((_ms < 0.75) || {
				(_isAdmin)
		...
		if ((_v isKindOf 'LandVehicle') || {
			...
			if (_n > 0) then {
				if (!isNil {
					_v getVariable 'QS_ST_mapClickShowCrew'
				}) then {
	...
	_QS_fnc_iconUnits = {
		...
		if (((_di isEqualTo 1) && ((_QS_ST_X select 65))) && {
			(!(_QS_ST_X select 75))
		}) then {
			_exit = true;
	...
	_QS_fnc_onMapSingleClick = {
		...
		if ((!(_alt)) && (!(_shift))) then {
			...
			_vehicles = (nearestObjects [_position, ['Air', 'LandVehicle', 'Ship'], 250, true]) select {
				(alive _x)
			};
		...
		if (_shift) then {
			if (player isEqualTo (leader (group player))) then {
				...
				_nearUnits = (nearestObjects [_position, ['CAManBase'], 250, true]) select {
					((alive _x) && ((group _x) isEqualTo (group player)) && (isNull (objectParent _x)))
				};
	...
	_QS_fnc_iconDrawGPS = {
		if ((!('MinimapDisplay' in ((infoPanel 'left') + (infoPanel 'right')))) ||
		{
			(visibleMap)
		} ||
		{
	...
	_QS_fnc_groupIconType = {
		...
		if ((_vehicleClass isEqualTo 'Ship') || {
			(_vehicleClass isEqualTo 'Submarine')
		}) exitWith {
		...
		if (_vehicleClass isEqualTo 'Armored') exitWith {
			if (['apc', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString')) then {
				...
				if ((['arty', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString')) || {
					(['mlrs', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString'))
				}) then {
		...
		if (_vehicleClass isEqualTo 'Support') exitWith {
			if (['medical', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString')) then {
				...
				if ((['ammo', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString')) || {
					(['box', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString'))
				} || {
					(['fuel', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString'))
				} || {
					(['CRV', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString'))
				} || {
					(['repair', _grpVehicle_type, FALSE] call (missionNamespace getVariable 'BIS_fnc_inString'))
				}) then {
	...
	_QS_fnc_iconColorGroup = {
		...
		if (_ps isEqualTo east) exitWith {
			_c = _QS_ST_X select 9; _u setVariable ['QS_ST_groupIconColor', [_c, _ps], false];_c;
		};
		if (_ps isEqualTo west) exitWith {
			_c = _QS_ST_X select 10; _u setVariable ['QS_ST_groupIconColor', [_c, _ps], false];_c;
		};
		if (_ps isEqualTo resistance) exitWith {
			_c = _QS_ST_X select 11; _u setVariable ['QS_ST_groupIconColor', [_c, _ps], false];_c;
		};
		if (_ps isEqualTo civilian) exitWith {
			_c = _QS_ST_X select 12; _u setVariable ['QS_ST_groupIconColor', [_c, _ps], false];_c;
		};
	...
	_QS_fnc_onGroupIconClick = {
		...
		if (!((side _group) isEqualTo playerSide)) exitWith {
			hintSilent QS_ST_STR_text2;0 spawn {
		...
		_AINames = _QS_ST_X select 72;
		{
			...
			if ([_x, ((_QS_ST_X select 15) select 0)] call (_QS_ST_X select 69)) then {
				_color = _colorIncapacitated;
			};
							...
							if (diag_tickTime > _groupUpdateDelay) then {
								{
									_grp = _x;
									if ((_showOwnGroup) || {
										((!(_showOwnGroup)) && (!(_grp isEqualTo (group player))))
									} || {
										(!(_showMapUnitIcons))
									}) then {
										if (({
											...
											if ((side _grp) in _as) then {
												...
												if (!isNil {
													_grp getVariable 'QS_ST_Group_Icon'
												}) then {
